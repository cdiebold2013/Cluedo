...
public class MainGameUI extends Application
{
    ...
    public void start(Stage primaryStage)
    {
        ...
        startButton.setOnAction(event -> {
            //start game
           // client.sendMessage(new ClueLessMsg(ClueLessMsg.START, ""));       //send the ClueLessMsg obj to server to start the game
            window.setScene(scene2);
        ...
        characterLB.setText(character);   //set this label accordingly
        characterLB.setFont(Font.font("Verdana", 15));

        charHB.getChildren().addAll(youAreLB, characterLB, charTokens2[playerID]);
        charHB.setAlignment(Pos.CENTER_LEFT);
        Label playerIDLB = new Label("Player ID:");
        idLB = new Label();
        idLB.setText(Integer.toString(playerID));
        HBox idHB = new HBox();
        ...
        String[] cards = { "Mrs White", "Rope"};
        cardsRBVBox = setCards(cards);                                     //display cards
        ...
        disproveHB.setVisible(false);                                                    //when it's the player's turn to disprove
    ...
    public VBox setCards(String[] cards) {

       return cardsRBVBox = setCardsRB(cards);
        }
